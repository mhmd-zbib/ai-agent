...
FILE = Path(__file__).parent / "test.txt"   # swap to any .pdf / .docx / .txt
...
def watch(document_id, timeout=180):
    ...
    STAGES = ["uploaded","parsing","parsed","chunking","chunked","embedding","storing","completed"]
...
content_type = (
    "application/pdf"           if FILE.suffix == ".pdf"  else
    "application/vnd.openxmlformats-officedocument.wordprocessingml.document"
                                if FILE.suffix == ".docx" else
    "text/plain"
)
parts = [
...
print(f"\n  {'═'*54}")
print(f"  RAG Pipeline Simulation")
...
print("[1] Register")
try:
    http("POST", f"{API_BASE}/v1/users/register", {"email": EMAIL, "password": PASSWORD})
    print("    registered")
...
# 2. Login
print("[2] Login")
token = http("POST", f"{API_BASE}/v1/users/login", {"email": EMAIL, "password": PASSWORD})["access_token"]
print(f"    token: {token[:32]}…")

# 3. Initiate upload
print("[3] Initiate upload")
resp      = http("POST", f"{API_BASE}/v1/documents/uploads",
                 {"file_name": FILE.name, "content_type": content_type,
                  "file_size_bytes": len(file_bytes), "chunk_size_bytes": CHUNK_BYTES},
                 token=token)
...
resp = http("POST", f"{API_BASE}/v1/documents/uploads/{upload_id}/complete",
            {"file_name": FILE.name, "content_type": content_type,
             "chunks": [{"chunk_index": i, "size_bytes": len(parts[i])} for i in range(len(parts))]},
            token=token)

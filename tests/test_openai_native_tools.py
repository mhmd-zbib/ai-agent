...
def test_openai_native_tools_content_only(openai_client, sample_tools):
    ...
    payload = AgentInput(
        user_message="How are you?", session_id="test-session", history=[]
    )
...
def test_openai_multiple_tool_calls_selects_first_valid(openai_client, sample_tools):
    ...
    payload = AgentInput(
        user_message="Check weather", session_id="test-session", history=[]
    )
    response = openai_client.generate(payload, response_mode="chat", tools=sample_tools)
...
def test_openai_multiple_tool_calls_skips_invalid_and_uses_next_valid(
    openai_client, sample_tools
):
...
def test_openai_multiple_tool_calls_all_invalid_raises_error(
    openai_client, sample_tools
):
...
def test_openai_chat_mode_normalizes_json_text_content(openai_client, sample_tools):
    ...
    payload = AgentInput(
        user_message="Weather in Paris?", session_id="test-session", history=[]
    )
    response = openai_client.generate(payload, response_mode="chat", tools=sample_tools)

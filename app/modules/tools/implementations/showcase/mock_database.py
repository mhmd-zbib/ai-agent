...
class MockDatabaseTool(BaseTool):
    ...
    def run(self, arguments: dict[str, Any]) -> str:
        ...
        if not isinstance(limit, int) or limit < 1 or limit > 100:
            error_msg = f"Error: Limit must be an integer between 1 and 100, got: {limit}"
            logger.warning(error_msg)
            return json.dumps({"error": error_msg}, indent=2)
        ...
        if table_name not in self.MOCK_TABLES:
            ...
            logger.warning(f"{error_msg}. Available tables: {list(self.MOCK_TABLES.keys())}")
            return json.dumps({
                "error": error_msg,
        ...
        logger.info(f"Generated {len(results['data'])} mock results for table {table_name}")
    ...
    def _generate_mock_results(self, table_name: str, limit: int, offset: int) -> dict[str, Any]:
        """

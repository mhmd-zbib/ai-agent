"""
agents.document.tools.extractor — DocumentLookupTool: semantic search over user documents.

Migrated from packages/api/src/api/modules/tools/implementations/document_lookup.py.
Import path changes:
  api.modules.tools.base       -> agents.core.tool
  api.modules.tools.exceptions -> agents.core.tool
"""

from __future__ import annotations

from agents.core.tool import BaseTool, ToolConfigurationError, ToolValidationError

__all__ = ["DocumentLookupTool"]
...
class DocumentLookupTool(BaseTool):
    ...
    def __init__(self, vector_client: object = None, embedding_client: object = None) -> None:
    ...
    def run(self, arguments: dict[str, object]) -> str:
        ...
        if not query:
            raise ToolValidationError(
                tool_id=self.name, validation_errors=["Query is required"]
            )
        ...
        query_vector = self._embedding_client.embed(query)  # type: ignore[union-attr]
        results = self._vector_client.query(  # type: ignore[union-attr]
            vector=query_vector,
            top_k=5,
            namespace=user_id,
        )
